...
async function getSubscription() {
  const endPoint = `https://www.googleapis.com/youtube/v3/subscriptions?part=snippet&channelId=UC6wLgjFDStkG6LpxidBl2AQ&key=${API_key}&maxResults=20`;
  try {
    const response = await fetch(endPoint);
    const result = await response.json();
    console.log(result);
...
async function loadVideosOntoHomePage() {
  const endPoint = `${baseURL}/videos?part=snippet&chart=mostPopular&regionCode=IN&key=${API_key}&maxResults=20&pageToken=${nextPageToken}`; //CBQQA
  try {
    const response = await fetch(endPoint);
    const result = await response.json();
    console.log(result);
    nextPageToken = result.nextPageToken; // CBQQA
